...
> Этот документ — справочник для уже установившего интеграцию. Установка, авторизация (Сбер ID и SMS), [параметры интеграции](README.md#параметры-интеграции), [удаление](README.md#удаление), [решение проблем](README.md#решение-проблем), [известные ограничения](README.md#известные-ограничения), благодарности, лицензия — в [README.md](README.md).
...
- Интервал polling настраивается: Настройки → Устройства и службы → SberHome → **Настроить** (10–3600 секунд, по умолчанию 30). Там же размер буферов DevTools (10–5000 записей, по умолчанию 200) и ожидание подтверждения команды (1–120 секунд, по умолчанию 10) — подробно в [README.md → Параметры интеграции](README.md#параметры-интеграции).
- Если облако Сбера не отвечает, сущности SberHome (в том числе уведомления «Sber TTS/TTC», сценарии и «Я дома») недоступны до следующего удачного опроса или изменения, пришедшего по WebSocket. В журнал пишется одна запись при потере связи и одна при её возвращении — отдельно для опроса и для WebSocket.
- Команда на несколько сущностей одного типа за один вызов (например, `light.turn_off` для всей комнаты) уходит в облако по очереди, по одной сущности.
...
Сервисы `refresh`, `reload_intents`, `send_raw_command`, `tts_send` и `ttc_send` доступны сразу после запуска Home Assistant, даже если запись SberHome ещё не загрузилась. Все они принимают необязательное поле `config_entry_id` — запись интеграции, к которой адресован вызов. Без него используется та же запись, с которой работает панель SberHome (обычно она единственная).

**Ошибки.** Если вызов не удался, сервис завершается ошибкой с переведённым сообщением: запись SberHome не найдена или не загружена, облако Сбера недоступно или отклонило команду, колонки не найдены и т.п. Автоматизация на этом шаге останавливается (или продолжает, если у шага стоит `continue_on_error: true`). В версиях до 5.43.0 включительно такие ошибки возвращались в ответе как `{"ok": false, "error": "..."}`, и вызов считался успешным. При успехе ответ, как и раньше, содержит `"ok": true` и данные сервиса.

```yaml
action:
  - service: sberhome.tts_send
    continue_on_error: true  # не останавливать автоматизацию, если колонка недоступна
    data:
      message: "Стиральная машина закончила"
```
...
Вызов ждёт окончания опроса облака (а также настроек колонок, если они доступны для вашего входа). Если опрос не удался — облако недоступно, ограничило частоту запросов, отвергло данные входа или вернуло ошибку, — сервис завершается ошибкой, и автоматизация не продолжит работу со старыми значениями.
...
Response payload содержит поля `intents_count`, `listeners_count` и отчёт синхронизации `results` (по `entry_id` записи). Если YAML не читается или содержит ошибку, сервис завершается ошибкой; если часть голосовых команд не удалось применить в облаке, — тоже ошибкой со списком их slug (подробности в журнале), а listeners при этом всё равно применяются.

Сервис может вызвать только администратор Home Assistant. Автоматизации вызывают его как раньше; скрипт — если его запустил администратор или автоматизация (запуск скрипта пользователем без прав администратора закончится ошибкой доступа).
...
Сервис может вызвать только администратор Home Assistant. Автоматизации вызывают его как раньше; скрипт — если его запустил администратор или автоматизация (запуск скрипта пользователем без прав администратора закончится ошибкой доступа). `device_id` — латинские буквы, цифры, `-` и `_` (до 128 символов). `state` — непустой список объектов (или тот же список строкой JSON), у каждого объекта обязателен строковый `key`; не больше 64 атрибутов и 16 КБ в JSON.
...
Сверху — карточка состояния: общий вердикт, устройства, опрос, WebSocket, срок действия токенов Сбер ID, Companion и умного дома (значения токенов не показываются), ошибки за сессию. Ниже — DevTools для отладки:
...
Внизу — журнал WS-сообщений: кольцевой буфер входящих и исходящих сообщений (и подставленных через Replay) с фильтрами по топику и направлению.

### Таб Debug

Один выбор устройства сверху и две подвкладки:

- **Payload** — разобранное состояние устройства и сырой JSON от Sber
- **Отправка команды** (Send command) — отправка команды (`sberhome.send_raw_command` через UI): форма по возможностям устройства или JSON-редактор, показ ответа

### Таб Settings

- Просмотр и изменение параметров интеграции — интервала опроса, размера буферов DevTools и ожидания подтверждения команды (те же, что в форме «Настроить», см. [README.md → Параметры интеграции](README.md#параметры-интеграции)); применяются без перезагрузки записи
- Кнопки повторного входа в панели нет: если облако перестало принимать токены, повторная авторизация запускается из **Настройки → Устройства и службы** (см. [README.md → Повторный вход](README.md#повторный-вход))

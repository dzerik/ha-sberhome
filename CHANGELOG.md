...
- **Переменная «я дома»** (`at_home`): чтение стало устойчивым к вложенному
  ответу `{variable:{value:{bool_value}}}` (раньше парсился только плоский
  `at_home` — состояние могло всегда показываться выключенным). Запись
  оставлена в исторически рабочем одноключевом формате. Оба вызова принимают
  опциональный `home_id`.

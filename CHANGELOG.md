...
## [6.2.0] — 2026-09-21 (beta)

### Added — кнопка «Дамп настроек колонки» в панели

- В панели SberHome (вкладка «Настройки колонки») появилась кнопка
  **«⬇ Дамп настроек»** — скачивает сырое дерево настроек колонки (`/v18`)
  JSON-файлом. Нужна для диагностики и добавления поддержки новых моделей
  колонок: сырой ответ сохраняется как есть, включая поля, которые
  интеграция пока не разбирает.

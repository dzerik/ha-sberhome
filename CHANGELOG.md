...
- `sberhome.tts_send` (из PR #48, @john1364) — озвучка `message` на выбранных
  колонках. Обходит строгую схему `notify.send_message` (HA 2023.7+ режет
  `device_ids` внутри `data`): здесь `device_ids` — обычный параметр сервиса.
- `sberhome.ttc_send` — то же для ассистент-команд (TTC): «Расскажи анекдот»,
  «Включи Bluetooth» и т.п. на выбранных колонках.

### Fixed — роутинг device→home (мультидом)

- Общий обработчик группирует `device_ids` по дому-владельцу через
  `state_cache.device_home_id`: команда уходит ТОЛЬКО в дом (и config entry),
  которому принадлежит колонка. Раньше `tts_send` слал во все дома всех entry
  — при нескольких домах это озвучивало не те колонки / слало чужие UUID.
  Без `device_ids` поведение прежнее: broadcast на все колонки всех домов.

...
### Fixed — колонка видна, а все сущности недоступны (issue #46)

- Убрав в 5.13.3 три `FeatureSpec`, я перестал создавать сущности, но записи о них
  остались в entity registry Home Assistant. HA показывает такие записи как
  «недоступно». У SberBox Time из четырёх сущностей выпускалась одна, три висели
  серыми; у SberBoom mini из пяти оставалось три.
- Теперь на старте интеграции снятые с производства сущности удаляются из реестра.
  Список снятых ключей лежит в `registry_maintenance.RETIRED_UNIQUE_ID_SUFFIXES`,
  добавление будущих — одна строка.
- Чистим строго по этому списку, а не по принципу «всё, чего нет в маппере»:
  light, climate, cover, fan, humidifier, media_player, vacuum и update собирают
  сущности своими ветками мимо `map_device_to_entities`, и сравнение с ним вынесло
  бы их подчистую. Тест на это есть.

## [5.13.3] — 2026-08-12

### Removed — сущности для настроек, которых облако не отслеживает (issue #46)

- Убраны переключатель «Звуки ассистента», выбор «Возрастной режим» и датчик «Геймпад»,
  добавленные в 5.13.0. Они не работали: переключение откатывалось, а смена настройки в
  приложении Сбера до Home Assistant не доходила.
- Причина видна по меткам `last_sync` в сыром JSON. На одной колонке `detector`
  обновляется в момент проверки, а все шесть ключей `staros_*` помечены одной меткой
  двухмесячной давности, одинаковой до миллисекунды. Это снимок настроек, а не поток
  значений. У `gamepad` метка вовсе `1970-01-01`: облако не получало его ни разу.
- Отсюда правило, записанное в коде рядом с реестром фич: списку `commands` от облака
  верить нельзя, надо смотреть `last_sync`. Облако объявляет команды и для того, чем не
  управляет.
- Распознавание SberBox Time из 5.13.0 остаётся: устройство появляется в Home Assistant,
  как и просили в issue #43.

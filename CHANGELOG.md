...
- Браузеру запрещено выполнять на этой странице любые скрипты, кроме её
  ...
  кэширует.
- Отправка адреса `companionapp://` принимает только запросы самой
  страницы. Сломанные или посторонние запросы получают понятный отказ, а
  не внутреннюю ошибку.
...
- **Если у вас уже несколько записей SberHome**, ни одна не удаляется и не
  ...
  продолжают работать. В разделе «Ремонт» Home Assistant появится
  замечание: с какой записью работает панель (первая загруженная) и какие

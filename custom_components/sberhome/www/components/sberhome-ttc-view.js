...
/**
 * 🧪 EXPERIMENTAL — TTC surrogate UI tab (команда ассистенту).
 * Третий segment в Automations panel. Статус surrogate-сценариев per home,
 * тестовая команда с latency feedback, автогенерированный YAML snippet.
 */
export class SberhomeTtcView extends LitElement {
  ...
  constructor() {
    ...
    this._message = "Расскажи анекдот";
  ...
  async _loadStatus() {
    this._loading = true;
    try {
      const r = await this.hass.callWS({ type: "sberhome/ttc_surrogate/status" });
      this._homes = r.homes || [];
      if (!this._selectedHomeId && this._homes.length) {
        this._selectedHomeId = this._homes[0].home_id;
      }
    } catch (err) {
      console.error("TTC surrogate status failed", err);
      this._homes = [];
    } finally {
      this._loading = false;
    }
  ...
  async _ensureSurrogate(homeId) {
    try {
      const r = await this.hass.callWS({
        type: "sberhome/ttc_surrogate/ensure",
        home_id: homeId,
      });
      if (r && r.ok === false) {
        console.error("TTC surrogate ensure rejected", r);
        alert("Не удалось создать surrogate: " + (r.error || "unknown error"));
        return;
      }
      await this._loadStatus();
    } catch (err) {
      console.error("TTC surrogate ensure failed", err);
  ...
  _homeSlug(name) {
    ...
    const lat = [...lower]
      .map((ch) => (SberhomeTtcView._CYR_TO_LAT[ch] ?? ch))
      .join("");
    return (
      lat
        .replace(/[^a-z0-9]+/g, "_")
  ...
  _renderTestForm() {
    ...
    return html`
      <div class="card">
        ...
        <div class="label" style="margin-top:8px;">Команда ассистенту</div>
  ...
  render() {
    ...
    return html`
      <div class="banner">
        ...
        <div class="body">
          Колонка ВЫПОЛНЯЕТ текст как команду ассистенту. Работает через run-time edit Sber-сценария. Каждый вызов = 2–3 API-call'а
      ...
      <div class="section-title">Тестовая команда</div>
...
customElements.define("sberhome-ttc-view", SberhomeTtcView);

...
class SberSbermapMediaPlayer(SberBaseEntity, MediaPlayerEntity):
    ...
    def state(self) -> MediaPlayerState | None:
        """Current state. None → HA показывает "unavailable" через
        `SberBaseEntity.available` (что корректнее чем OFF для недоступного
        устройства — OFF неотличим от "выключено вручную")."""
        ent = self._ent()
        if ent is None:
            return None
    ...
    async def async_turn_on(self) -> None:
        await self._async_send_attrs(build_tv_on_off_command(device_id=self._device_id, is_on=True))
    ...
    async def async_set_volume_level(self, volume: float) -> None:
        await self._async_send_attrs(
            build_tv_volume_command(device_id=self._device_id, volume_level=volume)
        )

    async def async_volume_up(self) -> None:
        await self._async_send_attrs(
            build_tv_volume_step_command(device_id=self._device_id, direction="+")
        )
    ...
    async def async_mute_volume(self, mute: bool) -> None:
        await self._async_send_attrs(build_tv_mute_command(device_id=self._device_id, mute=mute))
    ...
    async def async_send_direction(self, direction: str) -> None:
        await self._async_send_attrs(
            build_tv_direction_command(device_id=self._device_id, direction=direction)
        )

    async def async_play_channel(self, channel: int) -> None:
        await self._async_send_attrs(
            build_tv_channel_command(device_id=self._device_id, channel=channel)
        )

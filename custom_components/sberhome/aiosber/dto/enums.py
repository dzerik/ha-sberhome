...
Значения enum'ов собраны на основе наблюдения за REST/WS трафиком
Sber Gateway API и дополняются по мере появления новых значений.

Каждый enum хранит значение (то, что уходит/приходит в JSON
как `enum_value`) в `.value`. Конструктор `MyEnum(value_str)` парсит API.
...
не Boolean и не int. SdStatusAttr — единственный INTEGER enum.
"""
...
class LightOperationModeAttr(StrEnum):
    """Значения не восстановлены из APIа; используем имена в нижнем регистре."""
...
class SdFormatStatusAttr(StrEnum):
    """Значения не восстановлены из APIа; имена в нижнем регистре."""
...
# =============================================================================
# Domain HVAC modes (используется в spec, не в APIе как Attr)
# =============================================================================
...
class ScheduleDay(StrEnum):
    """Значения предположены lowercase (источник хранит const-имена)."""

...
class NameDto:
    """Имя устройства (field key: объект с полями name/default_name/names).

    Формат данных REST: ``{"name": {"name": "Люстра", "defaultName": "", "names": {}}}``
    ...
    def from_dict(cls, data: dict[str, Any] | str | None) -> Self | None:
        ...
        if isinstance(data, dict):
            # uses camelCase (defaultName), DTO uses snake_case (default_name).
...
class DeviceInfoDto:
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> Self | None:
        if data is None:
            return None
        # JSON key: model может быть dict {"model": "X", "manufacturer": "Y"}
...
class IndicatorColor:
    ...
    API-поля: id (UUID), hue, saturation, brightness.
    """

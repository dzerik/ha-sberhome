...
JSON schema: поле ``attributes`` в DeviceDto (массив объектов).
...
Структура восстановлена из JSON-ответов Sber Gateway REST API.
"""
...
class ScheduleValues:
    """Schedule attribute marker (no additional constraints in API)."""
...
class DeviceFeatureDto:
    ...
    JSON key: элемент массива ``attributes`` в DeviceDto.
    """

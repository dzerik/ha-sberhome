...
class DeviceService:
    ...
    def list_by_category(self, category: str) -> list[DeviceDto]:
        ...
        return [
            d
            for d in self._cache.get_all_devices().values()
    ...
    async def turn_on(self, device_id: str) -> None:
        await self.set_state(device_id, [AttributeValueDto.of_bool(AttrKey.ON_OFF, True)])

    async def turn_off(self, device_id: str) -> None:
        await self.set_state(device_id, [AttributeValueDto.of_bool(AttrKey.ON_OFF, False)])
    ...
    async def move_to_group(self, device_id: str, group_id: str | None) -> None:

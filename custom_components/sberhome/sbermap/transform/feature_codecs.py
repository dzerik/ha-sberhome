"""Feature codecs — single source of truth для API↔HA конверсии per feature.
...
вся API-семантика конкретной feature живёт ровно в одном месте.

**Verified API-семантика** (из API analysis + MQTT-SberGate sister project):
- `temperature` (sensor_temp / hvac) → INTEGER × 10 (22.5°C → 225 API).
...
class FeatureCodec(Protocol):
    """Bidirectional API↔HA конвертер для одной Sber-feature."""
...
class IntegerCodec:
    ...
    def to_ha(self, sber_value: Any) -> int | None:
        ...
        try:
            ...
            # Защита от mismatch'а codec↔API-type: если Sber для этого
            # устройства прислал ENUM ("low"/"medium"/"high") вместо INTEGER —
            # не валим весь refresh, а возвращаем None (feature просто не
            # появится как entity). Предотвращает полную потерю интеграции
            # из-за одного нестандартного устройства.
            return None
...
class TemperatureCodec:
    """Temperature: API FLOAT °C → HA float Celsius, direct pass-through.

    В клиентском SDK Sber все temperature-поля хранятся как `Float`
    (см. TemperatureSensorStateConverter, ThermostatStateConverter,
    HvacRadiatorStateConverter — везде `Float.valueOf(value)`). Никакого
    ×10 scaling нет. Раньше мы делили на 10, из-за чего 24°C
    отображались как 2.4°C — коммит с ×10 был ложной гипотезой.
    """
...
# Single source of truth — все API↔HA конверсии для каждой Sber-feature.
# ============================================================================
FEATURE_CODECS: dict[str, FeatureCodec] = {
    # ---- Temperature/humidity/pressure (sensor_temp + climate) ----
    "temperature": TemperatureCodec(),  # CRITICAL: API INTEGER × 10
    ...
    "hvac_heating_rate": EnumCodec(entity_category=EntityCategory.CONFIG, icon="mdi:speedometer"),
    "hvac_direction_set": EnumCodec(icon="mdi:arrow-decision"),
    "hvac_night_mode": BoolCodec(entity_category=EntityCategory.CONFIG, icon="mdi:weather-night"),
    "hvac_ionization": BoolCodec(entity_category=EntityCategory.CONFIG, icon="mdi:flash"),
    "hvac_aromatization": BoolCodec(entity_category=EntityCategory.CONFIG, icon="mdi:scent"),
    "hvac_decontaminate": BoolCodec(entity_category=EntityCategory.CONFIG, icon="mdi:shield-sun"),
    # ---- Light ----
    ...
    "open_percentage": IntegerCodec(unit_of_measurement=PERCENTAGE, suggested_display_precision=0),
    "open_state": EnumCodec(),
    "open_rate": EnumCodec(entity_category=EntityCategory.CONFIG, icon="mdi:speedometer"),
    ...
    # `signal_strength` в Sber — ENUM ("low"/"medium"/"high"), НЕ INTEGER.
    # Numeric dBm приходит в отдельной feature `signal_strength_dbm`.
    # Раньше мы маппили `signal_strength` как IntegerCodec → crash
    # `int("low")` для `dt_bulb_e27_m`.
    "signal_strength": EnumCodec(entity_category=EntityCategory.DIAGNOSTIC),
    "signal_strength_dbm": IntegerCodec(
    ...
    # ---- Switches ----
    "on_off": BoolCodec(),
    "child_lock": BoolCodec(entity_category=EntityCategory.CONFIG, icon="mdi:lock"),
    # ---- TV / Media player ----
...
def to_ha(feature: str, sber_value: Any) -> Any:
    """Конверт API → HA для feature. None если codec неизвестен."""
...
def to_sber(feature: str, ha_value: Any) -> Any:
    """Конверт HA → API для feature. None если codec неизвестен."""
    codec = FEATURE_CODECS.get(feature)
    if codec is None:

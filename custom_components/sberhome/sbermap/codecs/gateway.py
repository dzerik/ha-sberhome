"""GatewayCodec — wire-format приватного API `gateway.iot.sberdevices.ru`.

Конвенции:
- INTEGER → int (`220`).
- COLOR: type=`"COLOR"`, поле `color_value`, components `{hue 0..359,
...
class GatewayCodec:
    ...
    def decode_value(self, raw: dict[str, Any]) -> SberValue:
        ...
        try:
            ...
            raise CodecError(
                self.name, "decode", f"unknown wire type: {wire_type!r}", payload=raw
            ) from err
        ...
        if vt is ValueType.COLOR:
            ...
            if not isinstance(cv, dict):
                raise CodecError(
                    self.name, "decode", "COLOR missing color_value dict", payload=raw
                )
    ...
    def decode_color(self, raw: dict[str, Any]) -> HsvColor:
        return HsvColor(
            hue=int(raw.get("hue", 0)),
            saturation=int(raw.get("saturation", 0)),
            brightness=int(raw.get("brightness", 0)),
        )

...
@websocket_api.websocket_command({vol.Required("type"): "sberhome/get_devices"})
@callback
def ws_get_devices(
    hass: HomeAssistant,
    ...
    for device_id, dto in coord.devices.items():
        ...
        ha_device = find_ha_device(hass, coord, device_id)
        ...
        icon_path = (
            images.get("list_on")
            or images.get("cards_3d_on")
            or images.get("launcher_small_box_on")
        )
        out.append(
            {
                "device_id": device_id,
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "sberhome/refetch_device",
        vol.Required("device_id"): str,
    }
)
@websocket_api.async_response
async def ws_refetch_device(
    hass: HomeAssistant,
    connection: websocket_api.ActiveConnection,
    msg: dict[str, Any],
) -> None:
    """GET /devices/{id} — individual refresh из Sber (bypass batch tree).

    Если batch `/device_groups/tree` отдаёт stale значения для c2c-устройств
    (Tuya-bridge), этот endpoint дёргает single-device API напрямую —
    так же как делает мобильное приложение Sber.
    """
    coord = get_coordinator(hass)
    if coord is None:
        connection.send_error(msg["id"], "not_loaded", "Integration not loaded")
    ...
    connection.send_result(msg["id"], {"device_id": device_id, "raw_payload": raw})

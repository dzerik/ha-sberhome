...
class SberSbermapVacuum(SberBaseEntity, StateVacuumEntity):
    # VacuumEntityFeature.BATTERY и .STATE — deprecated в HA 2024+
    # (battery → отдельный SensorEntity через sbermap; state → `activity`).
    ...
    async def async_return_to_base(self, **kwargs: Any) -> None:
        await self._async_send_attrs(
            build_vacuum_command(device_id=self._device_id, command="return_to_base")
        )

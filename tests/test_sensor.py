...
class TestSignalStrengthSensor:
    """`signal_strength` — ENUM (low/medium/high). Numeric dBm теперь
    в отдельной feature `signal_strength_dbm`."""
    ...
    def test_native_value(self, entity):
        # ENUM value как есть — HA покажет строку "high" в UI.
...
class TestSignalStrengthDbmSensor:
    """Numeric dBm — для device_class=SIGNAL_STRENGTH + unit dBm."""
    ...
    def entity(self, mock_coordinator_with_entities):
        return _sensor_by_id(
            mock_coordinator_with_entities,
            "device_door_1",
            "device_door_1_signal_strength_dbm",
        )

    def test_unique_id(self, entity):
        assert entity._attr_unique_id == "device_door_1_signal_strength_dbm"

    def test_native_value(self, entity):
        assert entity.native_value == -40

    def test_unit_dbm(self, entity):
        assert entity._attr_native_unit_of_measurement == "dBm"
...
class TestVoltageSensor:
    ...
    def test_native_value(self, entity):
        # Sber API: INTEGER в Volts напрямую (PR #10).
...
class TestPowerSensor:
    ...
    def test_native_value(self, entity):
        # Sber API: INTEGER в Watts напрямую (PR #10).
        assert entity.native_value == 33


class TestNativeValueMissing:
    def test_returns_none_when_entity_disappears(self, mock_coordinator_with_entities):
        ent = _sensor_by_id(
            mock_coordinator_with_entities,
            "device_climate_1",
            "device_climate_1_temperature",

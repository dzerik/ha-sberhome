...
class TestClimate:
    def test_off_mode(self):
        ...
        b = build_climate_set_hvac_mode_command(device_id="d", hvac_mode=HVACMode.OFF, config=cfg)
        assert _val(b, "on_off") is False
        assert _val(b, "hvac_work_mode") is None

    def test_cool_mode(self):
        cfg = climate_config_for("hvac_ac")
        b = build_climate_set_hvac_mode_command(device_id="d", hvac_mode=HVACMode.COOL, config=cfg)
        assert _val(b, "on_off") is True
        assert _val(b, "hvac_work_mode") == "cool"

    def test_heater_no_modes_just_on(self):
        cfg = climate_config_for("hvac_heater")
        b = build_climate_set_hvac_mode_command(device_id="d", hvac_mode=HVACMode.HEAT, config=cfg)
        assert _val(b, "on_off") is True
        assert _val(b, "hvac_work_mode") is None

    def test_set_temperature(self):
        cfg = climate_config_for("hvac_ac")
        b = build_climate_set_temperature_command(device_id="d", temperature=22.0, config=cfg)
        assert _val(b, "hvac_temp_set") == 22

    def test_set_fan_mode(self):
        cfg = climate_config_for("hvac_ac")
        b = build_climate_set_fan_mode_command(device_id="d", fan_mode="high", config=cfg)

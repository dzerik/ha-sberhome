...
| `codecs/` | ❌ запрещены | wire-format Sber-only, нет HA-логики |
| `spec/_generated/` | ❌ запрещены | auto-gen константы из spec.json |
...
def test_homeassistant_only_in_allowed_paths():
    ...
    assert not violations, (
        "homeassistant imports вне разрешённых путей:\n" + "\n".join(violations)
    )

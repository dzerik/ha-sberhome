...
Покрывает дополнительные binary_sensor entities (tamper, water_low_level,
...
class TestKettleWaterLowLevel:
    ...
    def water_low(self, coordinator):
        return _bs(coordinator, "device_kettle_1", "device_kettle_1_water_low_level")
...
class TestHumidifierProblemSensors:
    def test_water_low(self, coordinator):
        e = _bs(coordinator, "device_humidifier_1", "device_humidifier_1_water_low_level")
    ...
    def test_replace_filter(self, coordinator):
        e = _bs(coordinator, "device_humidifier_1", "device_humidifier_1_replace_filter")
        assert e._attr_device_class is BinarySensorDeviceClass.PROBLEM
    ...
    def test_replace_ionizator(self, coordinator):
        e = _bs(coordinator, "device_humidifier_1", "device_humidifier_1_replace_ionizator")
        assert e._attr_device_class is BinarySensorDeviceClass.PROBLEM
        assert e._attr_entity_category is EntityCategory.DIAGNOSTIC
...
class TestAirPurifierProblemSensors:
    def test_replace_filter(self, coordinator):
        e = _bs(coordinator, "device_hvac_purifier_1", "device_hvac_purifier_1_replace_filter")
        assert e._attr_device_class is BinarySensorDeviceClass.PROBLEM
        assert e._attr_entity_category is EntityCategory.DIAGNOSTIC

    def test_replace_ionizator(self, coordinator):
        e = _bs(coordinator, "device_hvac_purifier_1", "device_hvac_purifier_1_replace_ionizator")
        assert e._attr_entity_category is EntityCategory.DIAGNOSTIC
...
class TestAsyncSetupEntryExtra:
    ...
    async def test_creates_all_extra_binary_sensors(self, coordinator):
        ...
        assert "device_kettle_1_water_low_level" in ids
        assert "device_humidifier_1_water_low_level" in ids
        assert "device_humidifier_1_replace_filter" in ids
        assert "device_humidifier_1_replace_ionizator" in ids
        assert "device_hvac_purifier_1_replace_filter" in ids
        assert "device_hvac_purifier_1_replace_ionizator" in ids

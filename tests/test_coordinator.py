...
def coordinator(
    ...
    coord.intent_dispatcher = MagicMock()
    return coord
...
def test_start_ws_task_creates_background_task(coordinator):
    """_start_ws_task регистрирует WS-задачу + стартует intent poller
    (poller теперь живёт в IntentDispatcher — SOLID-рефакторинг)."""
    coordinator._ws_task = None
    fake_task = MagicMock()
    coordinator.hass.async_create_background_task = MagicMock(return_value=fake_task)
    coordinator.intent_dispatcher = MagicMock()

    coordinator._start_ws_task()

    assert coordinator.hass.async_create_background_task.call_count == 1
    assert coordinator._ws_task is fake_task
    coordinator.intent_dispatcher.start_poller.assert_called_once()
...
def test_start_ws_task_swallows_exception(coordinator):
    ...
    # poller стартует независимо (сам глотает свои ошибки).
    coordinator.intent_dispatcher.start_poller.assert_called_once()

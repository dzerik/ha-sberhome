...
async def test_save_then_load_roundtrip(mock_hass: MagicMock, mock_entry: MagicMock) -> None:
...
async def test_save_sberid_updates_entry_data_token(
    store: HATokenStore, mock_hass: MagicMock, mock_entry: MagicMock
) -> None:
    """Ротированные SberID токены пишутся в `entry.data["token"]`.

    Без этого при ротации refresh_token в памяти новый токен теряется
    между рестартами HA, и интеграция падает в reauth при следующем
    запросе к Sber.
    """
    ...
    await store.save_sberid(tokens)

    mock_hass.config_entries.async_update_entry.assert_called_once()
    call = mock_hass.config_entries.async_update_entry.call_args
    new_data = call.kwargs["data"]
    assert new_data["token"]["access_token"] == "NEW_ACCESS"
    assert new_data["token"]["refresh_token"] == "NEW_REFRESH"


async def test_save_sberid_preserves_other_fields(
    mock_hass: MagicMock, mock_entry: MagicMock
) -> None:
    """save_sberid не должен затирать посторонние ключи entry.data
    (например, companion_tokens или будущие conf-ключи)."""
    from custom_components.sberhome.aiosber.auth import SberIdTokens

    mock_entry.data = {
        "token": {"access_token": "old"},
        CONF_COMPANION_TOKENS: {"access_token": "companion", "expires_in": 3600},
        "unrelated_key": "keep_me",
    }
    store = HATokenStore(mock_hass, mock_entry)

    tokens = SberIdTokens(access_token="ROTATED", expires_in=3600)
    await store.save_sberid(tokens)

    new_data = mock_hass.config_entries.async_update_entry.call_args.kwargs["data"]
    assert new_data["token"]["access_token"] == "ROTATED"
    assert new_data[CONF_COMPANION_TOKENS]["access_token"] == "companion"
    assert new_data["unrelated_key"] == "keep_me"

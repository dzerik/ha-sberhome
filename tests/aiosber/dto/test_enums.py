"""Roundtrip всех wire-значений Sber-енумов."""
...
# Все enum-классы с wire-значениями
ALL_ENUMS = [
    cls
...
def test_wire_value_roundtrip(cls):
    ...
    for member in cls:
        wire = member.value
        assert cls(wire) is member, f"{cls.__name__}: {member} ≠ cls({wire!r})"
...
def test_str_wire_for_obfuscated_numeric_enums():
    """Antiflicker / MotionSensitivity / Nightvision / Decibel — wire это СТРОКА с цифрой."""
...
def test_workmode_colour_british_spelling():
    """light_mode = COLOR в коде, но wire = "colour" (британское)."""
...
def test_floor_sensor_type_mixed_case():
    """FloorSensorType — wire с маленькой 'k' внутри (NTC10k, не NTC10K)."""
    assert enums.FloorSensorTypeAttr.NTC10K.value == "NTC10k"


def test_connection_type_camel_case():
    """ConnectionType — wire CamelCase с префиксом ConnType."""

...
def test_all_28_categories_covered():
    ...
    assert len(cats) == 28
    expected = {
        "light", "led_strip", "socket", "relay",
        "sensor_temp", "sensor_water_leak", "sensor_door",
        "sensor_pir", "sensor_smoke", "sensor_gas",
        "curtain", "window_blind", "gate", "valve",
        "hvac_ac", "hvac_heater", "hvac_radiator", "hvac_boiler",
        "hvac_underfloor_heating", "hvac_fan", "hvac_air_purifier", "hvac_humidifier",
        "kettle", "vacuum_cleaner", "tv",
        "scenario_button", "intercom", "hub",
    }
...
def test_base_proxies():
    d = _dto("light", name="Кухня", serial_number="SN1", sw_version="2.0",
             reported=[
...
def test_base_feature_type():
    d = _dto("light")
    t = TypedDevice(d)
    assert t.feature_type("on_off") == "BOOL"
    assert t.feature_type("temperature") == "FLOAT"
    assert t.feature_type("nonexistent") is None
...
class TestSensors:
    ...
    def test_motion_sensor_motion_state_fallback(self):
        d = _dto("sensor_pir", reported=[
            {"key": "motion_state", "type": "BOOL", "bool_value": True},
        ])
        assert MotionSensorDevice(d).motion is True

    def test_smoke(self):
        d = _dto("sensor_smoke", reported=[
...
class TestHvac:
    ...
    def test_humidifier(self):
        d = _dto("hvac_humidifier", reported=[
...
class TestAppliances:
    ...
    def test_vacuum(self):
        d = _dto("vacuum_cleaner", reported=[
...
class TestMisc:
    def test_scenario_button_event(self):
        d = _dto("scenario_button", reported=[
...
# ============== Generated constants integration ==============
class TestGeneratedConstantsIntegration:
    def test_all_typed_categories_in_spec(self):
        """Каждая категория с typed-classом должна быть в _generated."""
        from custom_components.sberhome.aiosber._generated.category_features import (
            ALL_CATEGORIES,
        )
        for cat in all_categories():
            assert cat in ALL_CATEGORIES, f"{cat} not in spec ALL_CATEGORIES"

    def test_spec_categories_have_typed_class(self):
        """Каждая spec-категория имеет typed-class."""
        from custom_components.sberhome.aiosber._generated.category_features import (
            ALL_CATEGORIES,
        )
        for cat in ALL_CATEGORIES:
            assert class_for_category(cat) is not None, f"no class for {cat}"


# ============================================================================
# Coverage gap closures — добавлено после audit (см. research_docs/07).
# ============================================================================
...
class TestScenarioButtonConfig:
    ...
    def test_color_indicator_hsv(self):
        d = _dto("scenario_button", reported=[{
            "key": "color_indicator_on", "type": "COLOR",
...
# ============================================================================
# Audit-driven test: убедиться что НИ ОДНА spec feature не пропущена
# ============================================================================
class TestSpecCoverageRegression:
    """Защита от regression — все spec features любой категории должны быть
    либо typed-property, либо явно помечены `None` (command/set-only) в audit."""

    def test_audit_passes_zero_missing(self):
        """Audit должен показать 0 missing spec features."""
        import subprocess
        result = subprocess.run(
            ["python3", "tools/audit_coverage.py"],
            capture_output=True,
            text=True,
            cwd="/home/dzerik/IdeaProjects/ha-sberhome",
            timeout=30,
        )
        assert result.returncode == 0, result.stderr
        # Проверяем сводку
        report = result.stdout
        # Извлекаем TOTAL строку
        for line in report.split("\n"):
            if line.startswith("| **TOTAL**"):
                # | **TOTAL** |  | **185** | **167** | **0** | **0** |
                parts = [p.strip() for p in line.split("|")]
                missing_spec = parts[5].replace("**", "").strip()
                assert missing_spec == "0", f"Audit reports missing spec features: {line}"
                break
        else:
            raise AssertionError("Could not find TOTAL line in audit report")

...
# ---- on_sberid_refreshed callback ----
async def test_on_sberid_refreshed_called_when_sberid_rotates():
    """Если SberID истёк и прошёл refresh — callback вызывается с новыми токенами.

    Критично для HA: Sber ротирует refresh_token, и без persist'а новых
    токенов в entry.data после рестарта HA используется устаревший
    refresh_token → InvalidGrant → forced reauth пользователя.
    """
    expired = SberIdTokens(
        access_token="OLD_SID",
    ...
    def token_handler(req: httpx.Request) -> httpx.Response:
        # refresh_token endpoint возвращает ротированный refresh_token
        return httpx.Response(
            200,
            json={
                "access_token": "NEW_SID",
    ...
    async with client as http:
        mgr = AuthManager(
            http=http,
            store=store,
            sberid_tokens=expired,
            on_sberid_refreshed=on_refreshed,
        )
        await mgr.access_token()

    assert len(captured) == 1
    assert captured[0].access_token == "NEW_SID"
    assert captured[0].refresh_token == "NEW_RT"


async def test_on_sberid_refreshed_not_called_when_sberid_fresh():
    """Если SberID-токен ещё валиден — ротации не происходит, callback молчит."""
    fresh = SberIdTokens(access_token="SID", refresh_token="RT", expires_in=3600)
    store = InMemoryTokenStore()
    captured: list[SberIdTokens] = []
...
async def test_on_sberid_refreshed_callback_errors_do_not_break_auth():
    """Если callback падает (например, HA async_update_entry кинул исключение),
    auth flow не должен фэйлиться — persist это best-effort."""
    expired = SberIdTokens(
        access_token="OLD",
        refresh_token="OLD_RT",
        expires_in=10,
    ...
    async with client as http:
        ...
        # Не должно поднять RuntimeError
        token = await mgr.access_token()

...
# -----------------------------------------------------------------------------
# unique_id from JWT id_token sub
# -----------------------------------------------------------------------------
...
def test_extract_sub_returns_claim_from_id_token() -> None:
    """Happy path: sub корректно извлекается из JWT id_token."""
    token = _make_token_with_sub("user-7777")
    assert ConfigFlow._extract_sub(token) == "user-7777"


def test_extract_sub_returns_none_when_no_id_token() -> None:
    """Без id_token → None (graceful: unique_id не устанавливается,
    но интеграция работает)."""
...
def test_extract_sub_returns_none_on_invalid_jwt() -> None:
    """Невалидный JWT → None (PkceError подавлён, unique_id skipped)."""
...
def test_extract_sub_returns_none_when_sub_missing() -> None:
    """JWT валидный, но без sub claim → None."""
    token = _make_token_with_sub(None)
    assert ConfigFlow._extract_sub(token) is None


@pytest.mark.asyncio
async def test_step_finish_sets_unique_id_from_sub() -> None:
    """`async_step_finish` должен установить unique_id из JWT sub
    и aborted'ить если уже configured (защита от дублей)."""
    flow = _make_flow_with_source("user")
    flow.hass = MagicMock()
    flow.flow_id = "finish-1"
    flow._client = MagicMock()
    flow._client.token = _make_token_with_sub("user-42")
    flow._client.aclose = AsyncMock()
    ...
    flow.async_set_unique_id.assert_awaited_once_with("user-42")
    flow._abort_if_unique_id_configured.assert_called_once()
    flow._client is None or (await flow._client.aclose())  # идемпотентно


@pytest.mark.asyncio
async def test_step_finish_reauth_checks_unique_id_mismatch() -> None:
    """При reauth-flow `async_step_finish` должен проверить что
    sub совпадает с оригинальной записью (_abort_if_unique_id_mismatch)."""
    flow = _make_flow_with_source("reauth")
    flow.hass = MagicMock()
    flow.flow_id = "reauth-1"
    flow._client = MagicMock()
    flow._client.token = _make_token_with_sub("user-77")
    flow._client.aclose = AsyncMock()
    ...
    flow.async_set_unique_id.assert_awaited_once_with("user-77")
    flow._abort_if_unique_id_mismatch.assert_called_once_with(reason="wrong_account")


@pytest.mark.asyncio
async def test_step_finish_skips_unique_id_when_no_sub() -> None:
    """Если sub не извлекается — unique_id не устанавливается (graceful
    degradation), entry создаётся без unique_id check."""
    flow = _make_flow_with_source("user")
    flow.hass = MagicMock()
    flow.flow_id = "nosub-1"
    flow._client = MagicMock()
    flow._client.token = {"access_token": "at"}  # нет id_token
    flow._client.aclose = AsyncMock()

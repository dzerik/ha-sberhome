...
async def test_callback_view_success():
    ...
    with patch(
        ...
        {"test-flow": mock_client},
    ):
        response = await view.post(request)

    assert response.status == 200
    mock_client.authorize_by_url.assert_awaited_once_with(
        "companionapp://host?code=abc&state=xyz"
    )
...
async def test_callback_view_missing_flow_id():
    ...
    view = SberAuthCallbackView()
    request = _make_callback_request(
        {"url": "companionapp://host?code=abc"}
    )
...
async def test_callback_view_invalid_url():
    ...
    view = SberAuthCallbackView()
    request = _make_callback_request(
        {"flow_id": "test-flow", "url": "https://evil.com"}
    )
